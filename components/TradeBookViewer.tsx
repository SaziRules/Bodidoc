...
export default function TradeBookViewer({ src, title, isLightbox = false, onClose }: TradeBookViewerProps) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setIsMobile(mobile);
      const available = Math.max(cw - (mobile ? 32 : ARROW_PAD), 100);
  ...
  return (
    <div ref={containerRef} className="w-full select-none">
      ...
      {status === "ready" && scale > 0 && (
        <>
          <div className="relative flex items-center justify-center py-10 px-4 md:px-[72px]">
            ...
            {isLightbox && (
              <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[10000] flex items-center bg-white/10 backdrop-blur-md border border-white/15 rounded-full px-1 max-w-[calc(100%-2rem)]">
                ...
                <span className="text-[11px] tabular-nums text-white/50 px-2.5 border-x border-white/15 whitespace-nowrap">
                  {showRange ? `${dispL}–${dispR}/${numPages}` : `${dispL}/${numPages}`}
                </span>
                {[
          ...
          <div className="border-t border-[#112942]/8 dark:border-[#5b9ac8]/10">
            ...
            <div className="flex md:hidden items-center justify-between px-4 py-3 border-b border-[#112942]/6 dark:border-[#5b9ac8]/10">
              ...
              <span className="text-xs tabular-nums text-[#112942]/45 dark:text-fg-muted">{dispL} of {numPages}</span>
              <button onClick={flipNext} disabled={isLast}
                className="w-9 h-9 rounded-full bg-[#112942]/6 hover:bg-[#112942]/12 dark:bg-[#5b9ac8]/6 dark:hover:bg-[#5b9ac8]/12 border border-[#112942]/10 dark:border-[#5b9ac8]/15
                           flex items-center justify-center text-[#112942] dark:text-[#5b9ac8] disabled:opacity-25 transition-all cursor-pointer">
                <ChevR />
      ...
      {lightboxOpen && createPortal(
        ...
        >
          <div className="w-full max-w-[1400px]">
            <TradeBookViewer src={src} title={title} isLightbox onClose={() => setLightboxOpen(false)} />
          </div>
        </div>,
        document.body
      )}
